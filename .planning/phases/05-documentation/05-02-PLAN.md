...
must_haves:
  truths:
    - "User can find syntax and examples for all 5 subcommands"
    - "User can collapse/expand command details to reduce visual clutter"
    - "User can find solutions to common errors without extra clicks"
    - "User can resolve prerequisite errors using troubleshooting section"
  artifacts:
    - path: "README.md"
...
<task type="auto">
  ...
  <action>
Add "## Commands" section after Concepts with collapsible details blocks for each command.
...
Before writing each command's syntax, run `agent-session COMMAND --help` and use the EXACT syntax from help output. Do NOT invent flag names or syntax.
  </action>
  <verify>
README.md Commands section verification:
1. Contains 5 subsections (start, stop, restart, ls, rm)
2. Each uses `<details>/<summary>` HTML tags
3. Proper spacing (blank lines after summary and details tags)
4. Examples are copy-paste safe
5. **Consistency check:** For each command, compare syntax in README against `agent-session COMMAND --help` output:
   - Run: `./agent-session start --help` and verify README matches
   - Run: `./agent-session stop --help` and verify README matches
   - Run: `./agent-session restart --help` and verify README matches
   - Run: `./agent-session ls --help` and verify README matches
   - Run: `./agent-session rm --help` and verify README matches
  </verify>
  <done>
All 5 commands documented in collapsible detail blocks with syntax verified against --help output.
  </done>
</task>
...
Keep solutions actionable with exact commands.
  ...
  <verify>
README.md Troubleshooting section verification:
1. NOT wrapped in `<details>` tags (visible by default)
2. Contains at least 6 error entries
3. Each has symptom heading, cause, solution with commands, verification
4. Covers both Linux and macOS-specific issues
5. **PREREQUISITES.md consolidation check:** Verify these PREREQUISITES.md errors are present:
   - "podman: command not found" - present in troubleshooting
   - "Cannot connect to Podman" / socket errors (macOS) - present in troubleshooting
   - UID mapping / "unable to look up current user" errors - present in troubleshooting
  </verify>
  <done>
Troubleshooting section complete with common errors (including all PREREQUISITES.md errors) and solutions, not collapsed.
  </done>
</task>
...
6. Command syntax matches `agent-session --help` output for each command
7. PREREQUISITES.md error messages are covered in troubleshooting
</verification>

<success_criteria>
- README.md provides complete documentation for agent-session
- All commands documented with syntax verified against actual help output
- Common errors (including all PREREQUISITES.md errors) have clear troubleshooting paths
