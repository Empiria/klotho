...
1. Add AGENT_SPECIFIED tracking near the top defaults (around line 15-20):
```bash
AGENT_SPECIFIED="false"
```

2. Update the `-a|--agent)` case block to set the flag:
```bash
-a|--agent)
    AGENT_TYPE="$2"
...
3. Add agent discovery function (after show_help, before argument parsing):
```bash
...
4. Add build status check function:
```bash
...
5. Add interactive menu function that sets AGENT_TYPE:
```bash
...
6. **Integration point** - Locate the line `load_agent_config "$AGENT_TYPE" || exit 1` (around line 170-180) and add the menu call BEFORE it:
...
The flow is: flag parsing sets AGENT_TYPE (default "claude") and AGENT_SPECIFIED -> if not specified, select_agent_interactive() calls discover_agents() and sets AGENT_TYPE via menu -> load_agent_config() uses final AGENT_TYPE value.
...
- Invalid input re-shows menu with hint
  ...
  <verify>
    ...
    - AGENT_SPECIFIED tracking: `grep "AGENT_SPECIFIED" agent-session`
    - Menu called before load_agent_config: `grep -A2 "select_agent_interactive" agent-session | grep "load_agent_config"`
  </verify>
  <done>Interactive agent menu added with alphabetical sorting, build status display, and default selection on empty input. AGENT_TYPE flows from menu to load_agent_config.</done>
</task>
