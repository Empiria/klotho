...
must_haves:
  ...
  key_links:
    - from: "scripts/build.sh"
      to: "podman build"
      via: "image tag with klotho-${AGENT_NAME}:latest"
    - from: "klotho"
      to: "podman ps"
      via: "find_container detecting both klotho-* and legacy agent-* patterns"
---
...
1. Update image tag pattern in scripts/build.sh:
   - Line 124: "agent-session-${AGENT_NAME}:latest" -> "klotho-${AGENT_NAME}:latest"
   - Line 133: "-t agent-session-${AGENT_NAME}:latest" -> "-t klotho-${AGENT_NAME}:latest"
   - Line 137: "Build complete: agent-session-..." -> "Build complete: klotho-..."

2. Update XDG config path in load_agent_config() function (lines 39-63):
   - Line 42 currently has: `local user_config="$config_home/agent-session/agents/$agent/config.conf"`
   - Replace the user_config logic with dual-path support:
   ```bash
   ...
   # Prefer new path, fall back to old with deprecation warning
...
grep -n "agent-session" scripts/build.sh  # Should only find backward compat fallback in old_config
```
  </verify>
