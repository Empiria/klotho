...
Add test to main.rs:
```rust
let runtime = klotho::container::detect_runtime()?;
...
Run: cargo run -- ls (will test detection and listing before hitting todo!())
  ...
  <done>
Container abstraction works: runtime detection with fallback, container listing, status checks, start/stop/remove operations
  </done>
</task>
