...
<task type="auto">
  ...
  <files>Cargo.toml, src/commands/build.rs</files>
  <action>
Implement build command with spinner progress:

First, add regex-lite dependency to Cargo.toml:
```toml
[dependencies]
# ... existing deps ...
regex-lite = "0.1"
...
pub fn run(all: bool, agents: Vec<String>, no_cache: bool, runtime_override: Option<&str>) -> Result<()> {
...
```
  ...
  <done>Build command implemented with spinner progress, Cargo.toml updated with regex-lite dependency</done>
</task>
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Stop { name } => klotho::commands::stop::run(name, runtime_override),
        Commands::Restart { name } => klotho::commands::restart::run(name, runtime_override),
        Commands::Ls => klotho::commands::ls::run(runtime_override),
        Commands::Rm { force, name } => klotho::commands::rm::run(name, force, runtime_override),
        Commands::Build { all, agents } => klotho::commands::build::run(all, agents, false, runtime_override),
        Commands::Rebuild { all, agents } => klotho::commands::build::run(all, agents, true, runtime_override),
    }
}
```
