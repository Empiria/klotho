...
<task type="auto">
  <name>Task 2: Create installer script with correct repo path</name>
  <files>install.sh</files>
  <action>
Create installer script for curl | sh installation:

First, determine the correct repo path. Check the git remote:
```bash
git remote get-url origin
```

Then create install.sh with the correct REPO value (replace OWNER with the actual GitHub username/org):
...
main() {
    # Configuration - UPDATE THIS with actual repo owner before first release
    # Get from: git remote get-url origin
    REPO="owenrumney/klotho"  # Example: "username/klotho" or "org/klotho"
...
IMPORTANT: Before the first release, verify the REPO value matches the actual GitHub repository path. Run `git remote get-url origin` and extract the owner/repo part (e.g., "owenrumney/klotho" from "git@github.com:owenrumney/klotho.git").
  </action>
  <verify>
```bash
# Check script syntax
...
# Verify REPO is not placeholder
grep "^    REPO=" install.sh | grep -v "OWNER/klotho"
```
  </verify>
  <done>
Installer script created: detects platform, downloads correct binary, handles PATH warning, REPO value set correctly
  </done>
</task>
...
# Verify REPO is configured (not placeholder)
grep "REPO=" install.sh
```
</verification>
...
- REPO variable contains actual repository path (not placeholder)
</success_criteria>
