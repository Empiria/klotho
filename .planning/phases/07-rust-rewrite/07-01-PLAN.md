...
2. cargo run -- --help shows main help with all commands and --runtime flag
3. cargo run -- start --help shows start-specific help
4. cargo run -- build --help shows build-specific help
5. cargo run -- --runtime docker ls shows runtime flag is parsed
  </verify>
  <done>
CLI structure complete: all 7 commands defined with proper arguments, global --runtime flag for Docker fallback, help text matches or improves bash version
  </done>
</task>
...
- Global --runtime flag visible in --help with default "auto"
