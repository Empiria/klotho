...
must_haves:
  truths:
    ...
    - "Auto-build prompts user when image missing"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/commands/start.rs"
      ...
      pattern: "config::"
    - from: "src/commands/start.rs"
      to: "src/commands/build.rs"
      via: "auto-build on missing image"
      pattern: "build::run_build"
---
...
```rust
pub mod start;
pub mod build;  // Already implemented in plan 06
...
pub fn run(agent: Option<String>, name: String, paths: Vec<String>, runtime_override: Option<&str>) -> Result<()> {
    ...
    let agent_type = match agent {
        Some(a) => a,
        None => select_agent_interactive(runtime_override)?,
    };

    // Load agent config
    let config = load_agent_config(&agent_type)?;

    // Ensure image is built (uses build::run_build from plan 06)
...
fn select_agent_interactive(runtime_override: Option<&str>) -> Result<String> {
...
fn ensure_image_built(runtime: Runtime, agent: &str) -> Result<()> {
    ...
    if answer == "y" || answer == "yes" {
        // Build using the real build implementation from plan 06
...
fn main() -> Result<()> {
    ...
    // Convert runtime flag to Option for passing to commands
    ...
    match cli.command {
        Commands::Start { agent, name, paths } => {
            ...
            klotho::commands::start::run(agent, name, paths, runtime_override)
        }
        Commands::Stop { name } => {
        ...
        Commands::Build { all, agents } => {
            klotho::commands::build::run(all, agents, false, runtime_override)
        }
        Commands::Rebuild { all, agents } => {
            klotho::commands::build::run(all, agents, true, runtime_override)
        }
    }
}
```

Note: This plan depends on plan 06 (build command) being complete because ensure_image_built() calls build::run_build() for auto-building when image is missing. The dependency is reflected in depends_on.
  </action>
  <verify>
```bash
# Build succeeds
...
```
  ...
  <done>
Start command works: creates containers, mounts paths, handles legacy naming, interactive agent selection, attaches to Zellij, auto-builds missing images via build::run_build
  </done>
</task>
...
- Auto-build prompt works and calls build::run_build when user confirms
</success_criteria>
